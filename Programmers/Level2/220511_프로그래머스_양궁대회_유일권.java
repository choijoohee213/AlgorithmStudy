...
class Solution {
    ...
    public int[] solution(int n, int[] info) {
        ...
        int apeach = getApeach(info);
        makecase(n, info, 0, 0, 0, apeach);
    ...
    public void checkScore(int n, int cnt, int ryan, int apeach){
        if(n >= cnt && ryan >= apeach){
            if(n>cnt) target[10] += n-cnt;
            if(isRyanWin(ryan, apeach)){
                max = ryan-apeach;
                answer = target.clone();
            }
        }
